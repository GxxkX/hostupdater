...
local function prepare_content(ct)
	if http.prepare_content then
		http.prepare_content(ct)
	elseif http.prepare_context then
		http.prepare_context(ct)
	end
end

local function write_json(data)
	if http.write_json then
...
local function get_post_data()
	...
	local cl = tonumber(http.getenv("CONTENT_LENGTH") or "0")
	if cl > 0 then
		local data = http.read(cl)
...
function index()
	...
	entry({"admin", "services", "hostupdater", "source"}, alias("admin", "services", "hostupdater")).dependent = false

	entry({"admin", "services", "hostupdater", "status"}, call("action_status"))
	entry({"admin", "services", "hostupdater", "update"}, call("action_update"))
	entry({"admin", "services", "hostupdater", "restore"}, call("action_restore"))
	entry({"admin", "services", "hostupdater", "config"}, call("action_config"))
	entry({"admin", "services", "hostupdater", "log"}, call("action_log"))
	entry({"admin", "services", "hostupdater", "hosts"}, call("action_hosts"))

	entry({"admin", "services", "hostupdater", "source", "toggle"}, call("action_source_toggle"))
	entry({"admin", "services", "hostupdater", "source", "update"}, call("action_source_update"))
	entry({"admin", "services", "hostupdater", "source", "delete"}, call("action_source_delete"))
...
local function write_sources(list)
	if nixio.fs.mkdirr then
		nixio.fs.mkdirr("/etc/hostupdater")
	else
		nixio.fs.mkdir("/etc/hostupdater")
	end
...
function action_status()
	...
	if sys.process and sys.process.list then
		service_running = (sys.process.list()["hostupdater"] ~= nil)
	else
		service_running = (sys.call("pidof hostupdater >/dev/null 2>&1") == 0)
	end
...
function action_config()
	...
	if method == "POST" then
		...
		prepare_content("application/json")
		write_json({ enabled = enabled, interval = interval })
	end
end
...
function action_sources()
	...
	if method == "POST" then
		...
		if not data.name or not data.url then
			prepare_content("application/json")
			write_json({ success = false, error = "missing name or url" })
			return
		end
		...
		for _, s in ipairs(list) do
			if s.name == data.name then
				prepare_content("application/json")
				write_json({ success = false, error = "source exists" })
				return
			end
		end
		...
		prepare_content("application/json")
		write_json({ success = ok, error = err })
		return
	end
...
function action_source_toggle(name)
	...
	if not name or name == "" then
		prepare_content("application/json")
		write_json({ success = false, error = "missing name" })
		return
	end
	local list = read_sources()
	local found = false
	...
	local ok, err = write_sources(list)
	prepare_content("application/json")
	write_json({ success = found and ok or false, error = (found and err or "not found") })
end
...
function action_source_delete(name)
	...
	prepare_content("application/json")
	write_json({ success = removed and ok or false, error = (removed and err or "not found") })
end
